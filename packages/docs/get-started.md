...
// 引入样式
import 'weizui-ui/dist/style.css'
...
```vue
<template>
  <wz-button>我是 Button</wz-button>
</template>
```

**单个导入**

weizui-UI 提供了基于 ES Module 的开箱即用的 Tree Shaking 功能。
...
<script>
  import { ErButton } from ' wz-element'
  export default {
    components: { ErButton },
  }
</script>
